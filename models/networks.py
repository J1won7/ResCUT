...
class Downsample3D(nn.Module):
    def __init__(self, channels, pad_type='replicate', filt_size=3, stride=2, pad_off=0):
        ...
        self.filt_size = filt_size
        self.pad_off = pad_off
        base_pad = [int(1.0 * (filt_size - 1) / 2), int(np.ceil(1.0 * (filt_size - 1) / 2))]
        self.pad_sizes = [base_pad[0], base_pad[1], base_pad[0], base_pad[1], base_pad[0], base_pad[1]]
        self.pad_sizes = [pad_size + pad_off for pad_size in self.pad_sizes]
        self.stride = stride
        self.channels = channels
...
class Upsample3D(nn.Module):
    def __init__(self, channels, mode='trilinear'):
        super().__init__()
        self.channels = channels
        self.mode = mode

    def forward(self, inp):
        return F.interpolate(inp, scale_factor=2, mode=self.mode, align_corners=False)
...
def get_pad_layer_3d(pad_type):
    ...
    if pad_type == 'zero':
        return lambda pad: PadLayer(pad, 0.0)
    return PadLayer
...
def define_G(input_nc, output_nc, ngf, netG, norm='batch', use_dropout=False, init_type='normal',
    ...
    if netG == 'resnet_9blocks':
        ...
        net = ResnetGenerator3D(input_nc, output_nc, ngf, norm_layer=norm_layer_3d, use_dropout=use_dropout, no_antialias=no_antialias, no_antialias_up=no_antialias_up, n_blocks=9, opt=opt)
    elif netG == 'resnet_6blocks_3d':
        net = ResnetGenerator3D(input_nc, output_nc, ngf, norm_layer=norm_layer_3d, use_dropout=use_dropout, no_antialias=no_antialias, no_antialias_up=no_antialias_up, n_blocks=6, opt=opt)
    elif netG == 'resnet_4blocks_3d':
        net = ResnetGenerator3D(input_nc, output_nc, ngf, norm_layer=norm_layer_3d, use_dropout=use_dropout, no_antialias=no_antialias, no_antialias_up=no_antialias_up, n_blocks=4, opt=opt)
    elif netG == 'unet_128':
...
def define_D(input_nc, ndf, netD, n_layers_D=3, norm='batch', init_type='normal', init_gain=0.02, no_antialias=False, gpu_ids=[], opt=None):
    ...
    if netD == 'basic':  # default PatchGAN classifier
        ...
        net = NLayerDiscriminator3D(input_nc, ndf, n_layers=3, norm_layer=norm_layer_3d, no_antialias=no_antialias)
        ...
        net = NLayerDiscriminator3D(input_nc, ndf, n_layers_D, norm_layer=norm_layer_3d, no_antialias=no_antialias)
...
class PoolingF(nn.Module):
    ...
    def forward(self, x):
        if x.dim() == 4:
            ...
            pooled = F.adaptive_max_pool3d(x, 1)
        else:
            raise ValueError(f"Unsupported tensor rank for PoolingF: {x.dim()}")
        return self.l2norm(pooled)
...
class ReshapeF(nn.Module):
    ...
    def forward(self, x):
        if x.dim() == 4:
            ...
            x = F.adaptive_avg_pool3d(x, 4)
            x_reshape = x.permute(0, 2, 3, 4, 1).flatten(0, 3)
        else:
            raise ValueError(f"Unsupported tensor rank for ReshapeF: {x.dim()}")
    ...
    def forward(self, feats, num_patches=64, patch_ids=None):
        ...
        for feat_id, feat in enumerate(feats):
            ...
            if feat.dim() == 4:
                ...
                raise ValueError(f"Unsupported feature rank for PatchSampleF: {feat.dim()}")
            if num_patches > 0:
            ...
            if num_patches == 0:
                x_sample = x_sample.permute(0, 2, 1).reshape([B, x_sample.shape[-1], *spatial_shape])
...
class ResnetGenerator3D(nn.Module):
    """3D ResNet generator following the same CUT backbone layout."""
    ...
    def __init__(self, input_nc, output_nc, ngf=64, norm_layer=nn.BatchNorm3d, use_dropout=False, n_blocks=6,
        ...
        for i in range(n_downsampling):
            ...
            if no_antialias:
                model += [
                    nn.Conv3d(in_ch, out_ch, kernel_size=3, stride=2, padding=1, bias=use_bias),
                    norm_layer(out_ch),
                    nn.ReLU(True),
                ]
            else:
                model += [
                    nn.Conv3d(in_ch, out_ch, kernel_size=3, stride=1, padding=1, bias=use_bias),
                    norm_layer(out_ch),
                    nn.ReLU(True),
                    Downsample3D(out_ch),
                ]
        ...
        for i in range(n_downsampling):
            ...
            if no_antialias_up:
                model += [
                    nn.ConvTranspose3d(in_ch, out_ch, kernel_size=3, stride=2, padding=1, output_padding=1, bias=use_bias),
                    norm_layer(out_ch),
                    nn.ReLU(True),
                ]
            else:
        ...
        out_conv = nn.Conv3d(ngf, output_nc, kernel_size=7, padding=0)
        model += [out_conv]
        if not (opt is not None and getattr(opt, 'use_residual_learning', False)):
            model += [nn.Tanh()]
        self.model = nn.Sequential(*model)

        if opt is not None and getattr(opt, 'use_residual_learning', False):
